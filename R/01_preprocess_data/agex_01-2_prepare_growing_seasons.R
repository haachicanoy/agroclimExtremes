...
# Prepare growing seasons on agricultural production areas
# By: Harold Achicanoy
# WUR & ABC
...
# --------------------------------------------------------------- #
# Number of growing seasons per site at 0.25°
# --------------------------------------------------------------- #
if(!file.exists(paste0(out,'/agex_nseasons_25km_corrected.tif'))){
  
  # Number of growing seasons at ~1 km
  ...
  # Mask number-of-growing-season raster by agricultural production areas
  ...
  # The previous raster includes misclassified pixels. This means that regions
  # with one growing season include pixels with two growing seasons, and
  # vice versa, due to the resampling step.
  
  # To overcome this issue, we performed a spatial density analysis to identify
  # the threshold from which we can classify pixels as one or two growing
  # seasons accordingly.
  
  # Split seasons
  n_seasons_1 <- n_seasons # One season
  n_seasons_1[n_seasons_1 != 1] <- NA
  n_seasons_1_dfm <- terra::as.data.frame(x = n_seasons_1, xy = T, cell = T, na.rm = T) # Coordinates
  
  n_seasons_2 <- n_seasons # Two seasons
  ...
  # Get densities (probabilities) from 'One season' coordinates
  ...
  # Get densities from 'Two seasons' coordinates
  ...
  # Visualize outliers
  thr <- 0.000017 # Obtained after several iterations
  hist(n_seasons_dfm$probability[n_seasons_dfm$nseasons == 2]); abline(v = thr, col = 2, lty = 2)
  plot(n_seasons); points(n_seasons_dfm[n_seasons_dfm$nseasons == 2 & n_seasons_dfm$probability < thr,c('x','y')], col = 'red', pch = 20)
...
# --------------------------------------------------------------- #
# Starting and ending dates of growing season
...
# --------------------------------------------------------------- #
# Mask growing-season dates by agricultural production areas
# --------------------------------------------------------------- #
# One season
