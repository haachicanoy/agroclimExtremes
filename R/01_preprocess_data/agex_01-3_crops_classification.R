...
# Crop classification counts at 0.1° on agricultural production areas
# By: Harold Achicanoy
# WUR & ABC
...
for(i in 1:length(grps)){
  if(!file.exists(paste0(root,'/agex_raw_data/agex_',gsub(' ','_',grps[i]),'_count.tif'))){
    ...
    terra::writeRaster(x = crops_lnd, filename = paste0(root,'/agex_raw_data/agex_',gsub(' ','_',grps[i]),'_count.tif'), overwrite = T)
