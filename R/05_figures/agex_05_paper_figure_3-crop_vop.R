## ------------------------------------------ ##
## Paper figure 3: crop VOP vs drought intensification
## By: Harold Achicanoy
## WUR & ABC
## Sep 2024
## ------------------------------------------ ##
...
## Figure 3 ----
# Bivariate map of SPEI severity vs crop VOP (agriculture exposure)

# Crop VOP
crp_vop <- terra::rast(paste0(root,'/agroclimExtremes/agex_results/agex_vulnerability/food_vop.tif'))
...
# Cluster: 236 (Haiti,Dominican Republic,Puerto Rico,Jamaica)
# Two growing seasons. S1 selected
...
### Panel c) Africa ----
# Cluster: 340 (Tanzania,Uganda)
# Two growing seasons. S2 selected

idx_ts <- terra::rast(paste0(root,'/agroclimExtremes/agex_indices/agex_spei-6/agex_spei-6_25km/two_s2_spei-6_25km.tif'))
# Filter time series per cluster of interest
aux <- agex_sgn; aux[aux != 340] <- NA
idx_ts_msk <- terra::mask(x = idx_ts, mask = aux) |> terra::trim()
# Time series plotting
idx_ts_msk_dfm <- terra::as.data.frame(x = idx_ts_msk, xy = T, cell = F)
...
ggplot2::ggsave(filename = paste0(root,'/agroclimExtremes/agex_results/agex_figures/Figure3_paper1-crop_vop.png'), plot = fig3, device = 'png', width = 12, height = 10, units = 'in', dpi = 350)
rm(ggm,
