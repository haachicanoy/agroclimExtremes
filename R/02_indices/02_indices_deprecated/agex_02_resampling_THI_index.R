...
resampling_indices <- function(index = 'thi'){
  ...
  # List files
  fls <- list.files(path = paste0(root,'/agroclimExtremes/agex_raw_data/monthly_',index,'_10km'), pattern = paste0('^',toupper(index),'_max-*.*.tif$'), full.names = T)
  # Load indices at 10 km
  idx <- terra::rast(fls)
  names(idx) <- gsub('.tif','',basename(fls))
  # Croplands mask
  crp <- terra::rast(paste0(root,'/agroclimExtremes/agex_raw_data/agex_croplands_foods_mapspam_25km.tif'))
  ...
  outfile <- paste0(root,'/agroclimExtremes/agex_indices/agex_',index,'/agex_',index,'_25km/monthly_',index,'_25km.tif')
  ...
  outfile <- paste0(root,'/agroclimExtremes/agex_indices/agex_',index,'/agex_',index,'_25km/annual_',index,'_25km.tif')
  dir.create(dirname(outfile),F,T)
