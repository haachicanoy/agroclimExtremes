...
lapply(X = yrs, FUN = function(yr){
  ...
  names(rsl) <- c('p95','day')
  terra::writeRaster(x = rsl, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate/one_s1_y1_p95_',yr,'.tif'), overwrite = T); gc(T)
  return('Done.\n')
})
...
lapply(X = yrs, FUN = function(yr){
  ...
  if(yr < 2023){
    ...
    terra::writeRaster(x = rsl, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate/one_s1_y2_p95_',yr,'.tif'), overwrite = T); gc(T)
    return('Done.\n')
  } else {
    break
    return('Done.\n')
...
yrs <- 1979:2022
lapply(X = yrs, FUN = function(yr){
  fls <- list.files(pattern = paste0('_',yr,'.tif'), path = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate'), full.names = T)
  p95 <- lapply(fls, terra::rast)
  p95 <- terra::merge(x = p95[[1]], y = p95[[2]])
  terra::writeRaster(x = p95, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_10km/one_s1_p95_',yr,'.tif'), overwrite = T)
})
...
lapply(X = yrs, FUN = function(yr){
  ...
  rsl <- terra::rapp(x = rnf, first = s1_ini, last = s1_end, fun = p95); gc(T)
  names(rsl) <- c('p95','day')
  terra::writeRaster(x = rsl, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate/two_s1_y1_p95_',yr,'.tif'), overwrite = T); gc(T)
  return('Done.\n')
})
...
lapply(X = yrs, FUN = function(yr){
  ...
  if(yr < 2023){
    ...
    rsl <- terra::rapp(x = rnf, first = s1_ini, last = s1_end+dys, fun = p95)
    names(rsl) <- c('p95','day')
    terra::writeRaster(x = rsl, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate/two_s1_y2_p95_',yr,'.tif'), overwrite = T); gc(T)
    return('Done.\n')
  } else {
    break
    return('Done.\n')
...
yrs <- 1979:2022
lapply(X = yrs, FUN = function(yr){
  fls <- list.files(pattern = paste0('two_s1_y[0-9]_p95_',yr,'.tif'), path = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate'), full.names = T)
  p95 <- lapply(fls, terra::rast)
  p95 <- terra::merge(x = p95[[1]], y = p95[[2]])
  terra::writeRaster(x = p95, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_10km/two_s1_p95_',yr,'.tif'), overwrite = T)
})
...
lapply(X = yrs, FUN = function(yr){
  ...
  names(rsl) <- c('p95','day')
  terra::writeRaster(x = rsl, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate/two_s2_y1_p95_',yr,'.tif'), overwrite = T); gc(T)
  return('Done.\n')
})
...
lapply(X = yrs, FUN = function(yr){
  ...
  if(yr < 2023){
    ...
    names(rsl) <- c('p95','day')
    terra::writeRaster(x = rsl, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate/two_s2_y2_p95_',yr,'.tif'), overwrite = T); gc(T)
    return('Done.\n')
  } else {
    break
    return('Done.\n')
...
yrs <- 1979:2022
lapply(X = yrs, FUN = function(yr){
  fls <- list.files(pattern = paste0('two_s2_y[0-9]_p95_',yr,'.tif'), path = paste0(root,'/agex_indices/agex_p95/agex_p95_intermediate'), full.names = T)
  p95 <- lapply(fls, terra::rast)
  p95 <- terra::merge(x = p95[[1]], y = p95[[2]])
  terra::writeRaster(x = p95, filename = paste0(root,'/agex_indices/agex_p95/agex_p95_10km/two_s2_p95_',yr,'.tif'), overwrite = T)
})
